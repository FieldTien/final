...
for i in range(10):
    ...
    print("#"*50)
    print("Fold: ", i)
    model = models.load_model('model/crnn_best_%d.h5'%i)
...
X_test = np.load(X_test_path)
y_hat_aug=np.ones((9400,41))
for i in range(10):

    print("#"*50)
    print("Fold: ", i)
    model = models.load_model('model/cnn_a2_best_%d.h5'%i)

    # Save test predictions
    print ('Prediction on Testing')
    y_hat_aug = y_hat_aug*model.predict(X_test, batch_size=64, verbose=1)
    
y_hat_aug=y_hat_aug**(1/10)
